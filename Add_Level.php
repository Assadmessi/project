...
if (isset($_REQUEST['Mode'])) 
{
	...
	$LevelRow=mysql_fetch_array($ret);

	$LevelID=$LevelRow['LevelID'];
	$LevelName=$LevelRow['LevelName'];
	$CourseID=$LevelRow['CourseID'];
	$Fees=$LevelRow['Fees'];
...
if (isset($_POST['btnupdate'])) 
{
		...
		if ($ret) {
			...
			echo "<script>window.location='Add_Level.php'</script>";
		}
		else{
			echo mysql_error();
		}
...
{
$LevelID=AutoID('Level','LevelID','LID-',6);
$LevelName=$_POST['txtLN'];
$CourseID=$_POST['txtCID'];
$Fees=$_POST['txtFees'];
$Description=$_POST['txtDescription'];
...
if ($count<>0) 
{
	echo "<script>window.alert('Level Already Exists')</script>";
	echo "<script>window.location='Add_Level.php'</script>";
}
else
{
	$insert="INSERT INTO level (LevelID,LevelName,CourseID,Fees,Description) VALUES ('$LevelID','$LevelName','$CourseID','$Fees','$Description')";
	$insertret=mysql_query($insert);
	if ($insertret) 
	{
		echo "<script>window.alert('Level Successful Insterted!')</script>";
		echo "<script>window.location='Add_Level.php'</script>";
	}
	else
	{
		echo "<p>Error in Level Registration Page:".mysql_error()."</p>";
	}
}
}
?>
...
<html lang="zxx">
<head>
<title>Level Registration</title>
<!-- Meta-Tags -->
...
<div class="table">
	...
	<h3 class="tittle-agileits-w3layouts">Level Registration</h3>
	</div>
...
<table>
		<tr>
				<td>
			<input type="hidden" name="txtLID" value="<?php echo $LevelID  ?>" required/>
		</td>
	</tr>
		<tr>
		<td>Level Name :</td>
		<td>
			<input type="text" name="txtLN" value="<?php if (isset($LevelName)) {
				echo $LevelName;
			} else {$LevelName="";}?>" required/>
		</td>
	</tr>
	<tr>
	<td>Course ID</td>
	...
	<td>
		<select name="txtCID" class="Textbox1">
	...
	<tr>
		<td>Fees</td>
		<td>
			<input type="text" name="txtFees" value="<?php if (isset($Fees)) {
				echo $Fees;
			} else {$Fees="";}?>" required/>
		</td>
	</tr>
	<tr>
		<td>Description :</td>
		<td>
			<input type="text" name="txtDescription" value="<?php if (isset($Description)) {
				echo $Description;
...
<?php 
	...
	echo "<tr>
		...
		<td>$Fees</td>
		<td>$Description</td>";
	echo "<td><a href='Add_Level.php?LevelID=$LevelID&Mode=update'>Edit</a>
	<a href='Add_Level.php?LevelID=$LevelID&Mode=delete'>Delete</a></td></tr>";
}
	?>
</table>
</fieldset>	
