...
if (isset($_REQUEST['Mode'])) 
{
	...
	$RoomRow=mysql_fetch_array($ret);

	$RoomID=$RoomRow['RoomID'];
	$RoomNO=$RoomRow['RoomNO'];
...
{
$RoomID=AutoID('Room','RoomID','RID-',6);
$RoomNO=$_POST['txtRN'];
...
if ($count<>0) 
{
	echo "<script>window.alert('Room Already Exists')</script>";
	echo "<script>window.location='Add_Room.php'</script>";
}
else
{
	$insert="INSERT INTO room (RoomID,RoomNO) VALUES ('$RoomID','$RoomNO')";
	$insertret=mysql_query($insert);
	if ($insertret) 
	{
		echo "<script>window.alert('Room Successful Insterted!')</script>";
		echo "<script>window.location='Add_Room.php'</script>";
	}
	else
	{
		echo "<p>Error in Room Registration Page:".mysql_error()."</p>";
	}
}
}
?>
...
<div class="table">
	...
	<h3 class="tittle-agileits-w3layouts">Room Registration</h3>
	</div>
...
<table>
		<tr>
				<td>
			<input type="hidden" name="txtRID" value="<?php echo $RoomID  ?>" required/>
		</td>
	</tr>
		<tr>
		<td>Room No :</td>
		<td>
			<input type="text" name="txtRN" value="<?php if (isset($RoomNO)) {
				echo $RoomNO;
			} else {$RoomNO="";}?>" required/>
		</td>
	</tr>
	<tr>
			<td></td>
...
<h1 style="margin-left:30px;">Room List</h1>
<tr>
	<th>Room ID</th>
	<th>Room No</th>
</tr>
<?php 
	...
	echo "<tr>
		<td>$RoomID</td>
		<td>$RoomNO</td>";
	echo "<td><a href='Add_Room.php?RoomID=$RoomID&Mode=update'>Edit</a>
	<a href='Add_Room.php?RoomID=$RoomID&Mode=delete'>Delete</a></td></tr>";
}
	?>
	</table>
</fieldset>
