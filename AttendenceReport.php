...
if(!$connect)
{
	...
	$db=mysql_select_db("allsmileenglishclassdb");
	if(!$db)
	{
		echo "Error".mysql_error();
		}
...
<div class="table">
  ...
  <h3 class="tittle-agileits-w3layouts">Attendance Report</h3>
    ...
    <div class="col-md-9 test-gr">
     ...
     <tbody>
          <?php        
            $query=mysql_query("Select (Select count(*) from Attendence Where Status='P')/ count(StudentID) *100 as Percentage from Attendence ");

      $query3=mysql_query(" Select Distinct T.StudentID,st.StudentName,s.SectionID from attendence T 
                            INNER join Student st on st.StudentID=T.StudentID
     ...
     echo '<td>'. $StudentID . '</td>';
      ...
      echo '<td>'. $SectionID . '</td>';
        ...
        $select= "Select 
                            ...
                            FROM Attendence a, Student s,Section se
                            WHERE a.StudentID = s.StudentID
                            and se.SectionID=a.SectionID
                            ...
                            and se.SectionID=a.SectionID
                            and a.SectionID='$SectionID'
                            AND a.StudentID = '$StudentID') * 100 percentage
